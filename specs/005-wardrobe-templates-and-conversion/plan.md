...
- **Story B** introduces `WardrobeTemplate` (system-only, read-only to users)
  and `TemplateSlot` (per-user). Users pick one of the seeded templates and add
  category slots to it. Extra wardrobe items beyond the slot count are always
  visible in the wardrobe view. The frontend renders empty slots as placeholder
  cards interleaved with owned items.
...
**New tables**: `WardrobeTemplates` (system), `TemplateSlots` (per-user).

**New domain types**: `WardrobeTemplate` (read-only value), `TemplateSlot` aggregate.
...
- `20260611_AddWardrobeTemplatesAndSlots` — schema: both new tables, no
  `user_id` on `WardrobeTemplates`, UNIQUE index on `TemplateSlots.wardrobe_item_id`
- `20260611_SeedDefaultTemplates` — data: inserts "Capsula" and "Trabalho"
  with fixed UUIDs via `migrationBuilder.InsertData`; `Down()` deletes them

## Architectural Decisions

- **Templates are system-only value objects** — `WardrobeTemplate` has no
  `user_id` and exposes no mutation methods. No Create/Rename/Delete commands
  exist for templates. The repository only exposes `GetAll()`.
...
- **Auto-fulfillment as an application service** — `TemplateSlotFulfillmentService`
  is called by `CreateWardrobeItemHandler` and `ConvertWishlistItemHandler` after
  a successful item write. It queries open slots by `(user_id, category)` across
  all templates and assigns the oldest. This keeps the rule in one place.
...
Goal: read-only template query, per-user slot CRUD, auto-fulfillment hook,
slot reversion hook, and "Adicionar à Lista de Desejos" command.

Steps:
1. `WardrobeTemplate` value type (id, name) — no mutation methods.
2. `TemplateSlot` aggregate (Fulfill, Unfulfill, LinkToWishlist).
3. `TemplateSlotFulfillmentService` application service.
4. EF Core config + `20260611_AddWardrobeTemplatesAndSlots` migration.
5. `20260611_SeedDefaultTemplates` data migration.
6. `IWardrobeTemplateRepository` (GetAll only) + `ITemplateSlotRepository` (CRUD).
7. Application queries/commands: `GetTemplatesQuery`, `GetUserSlotsQuery`,
   `AddSlotCommand`, `RemoveSlotCommand`, `LinkSlotToWishlistCommand`.
8. Hook fulfillment into `CreateWardrobeItemHandler` and `ConvertWishlistItemHandler`.
9. Hook reversion into `DeleteWardrobeItemHandler`.
10. Write failing tests (unit, integration, contract) first, then implement.

### Phase 4 — Story B Frontend: Template UI

Goal: template selector, empty slot placeholders interleaved with owned items,
progress indicator, "Adicionar à Lista de Desejos" slot action. No template
management UI (create/rename/delete) — templates are system-defined.

Steps:
1. `wardrobeTemplatesApi.ts` service (GET templates, slot CRUD, link-to-wishlist).
2. Template selector dropdown in wardrobe view header ("Sem template" = default).
3. `TemplateSlotCard` placeholder component (unfulfilled and fulfilled states).
4. `TemplateProgressBar` ("N de M peças adquiridas").
5. Wardrobe view renders slot placeholders interleaved with owned items per category;
   extra items beyond slot count are always visible below.
6. "Adicionar à Lista de Desejos" action on `TemplateSlotCard`.
7. Update e2e tests.
...
- **Architecture Gate**: Repository and Result patterns for all new handlers; no
  DbContext in application layer; templates exposed as read-only value objects.
- **DB Versioning Gate**: Two timestamped migrations — schema first, then data.
- **Performance Gate**: Slot query is `(user_id, category)`-indexed; auto-fulfillment
  is a single UPDATE bounded by user's open slot count.
- **Secret Management Gate**: No new secrets.
- **UX Consistency Gate**: All new UI text in pt-BR; placeholder cards follow
  established card design; no template management controls exposed to users.
