...
public sealed class WardrobeMediaValidationContractTests
{
    [Theory]
    [InlineData("image/gif", 1024)]
    [InlineData("image/jpeg", 11 * 1024 * 1024)]
    public async Task UploadUrlWhenMediaConstraintsAreInvalidShouldReturnBadRequest(string contentType, long fileSizeBytes)
    {
        ...
        var action = await controller.CreateUploadUrlAsync(
            new CreateUploadUrlRequest("image.bin", contentType, fileSizeBytes, "WardrobeBodyImage"),
